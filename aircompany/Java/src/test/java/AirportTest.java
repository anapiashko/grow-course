...
public class AirportTest {
    ...
    public void testSortByMaxLoadCapacity() {
        ...
        boolean nextPlaneLoadCapacityIsHigherThanCurrent = true;
        for (int i = 0; i < planesSortedByMaxLoadCapacity.size() - 1; i++) {
            Plane currentPlane = planesSortedByMaxLoadCapacity.get(i);
            Plane nextPlane = planesSortedByMaxLoadCapacity.get(i + 1);
            if (currentPlane.getMaxLoadCapacity() > nextPlane.getMaxLoadCapacity()) {
                nextPlaneLoadCapacityIsHigherThanCurrent = false;
                break;
            }
        }
        Assert.assertTrue(nextPlaneLoadCapacityIsHigherThanCurrent);
    }
    ...
    @Test
    public void testHasExperimentalPlanesUnclassifiedLevel(){
        Airport airport = new Airport(planes);
